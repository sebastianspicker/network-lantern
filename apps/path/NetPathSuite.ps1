...
.EXAMPLE
  .\apps\path\NetPathSuite.ps1 -DryRun
  Preview the full test plan without running any diagnostics.

.EXAMPLE
  .\apps\path\NetPathSuite.ps1 -Protocols IPv4 -Rounds Standard -HostsIPv4 google.com
  Run only the Standard round over IPv4 against a single host.

.EXAMPLE
  .\apps\path\NetPathSuite.ps1 -SkipPathping -Quiet
...
if ($Version) {
  Write-Output "NetPathSuite.ps1 v$SuiteVersion"
  return
}
...
if (-not $IsWindowsRuntime -and -not $DryRun) {
  throw 'Full NetPathSuite diagnostics require Windows built-in tools (ping/tracert/pathping/Test-NetConnection). Use -DryRun on non-Windows hosts.'
}
