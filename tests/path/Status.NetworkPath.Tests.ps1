...
Describe 'Network path status modeling' {
  ...
  It 'marks optional port probes skipped when none are configured' {
    Mock Invoke-PingRaw { [pscustomobject]@{ Raw = @('ping'); ExitCode = 0 } }
    Mock Invoke-TracertRaw { [pscustomobject]@{ Raw = @('tracert'); ExitCode = 0 } }
    Mock Invoke-PathpingRaw { [pscustomobject]@{ Raw = @('pathping'); ExitCode = 0 } }
    Mock Test-TcpPort { [pscustomobject]@{ TcpTestSucceeded = $true } }
    Mock Test-UdpPortBestEffort { throw 'No optional port probe should run.' }

    $round = (Get-RoundDefinitions -TraceMaxHops 30 -TraceTimeoutMs 5000 -PathpingProbes 50 -PathpingTimeoutMs 3000 | Select-Object -First 1)
    $result = Invoke-HostDiagnostics -PlanItem ([pscustomobject]@{ RoundDef = $round; Protocol = 'IPv4'; Host = 'example.local' }) -PortTargets @() -Settings ([pscustomobject]@{ PingCount = 5; SkipPathping = $false })

    $result.Ports | Should -HaveCount 0
    $result.PortsStatus | Should -Be 'Skipped'
    $result.FailedStages | Should -Not -Contain 'PortProbes'
    $result.OverallStatus | Should -Be 'OK'
  }
  ...
  It 'neutralizes formula-prefixed host names in CSV output' {
    $jsonPath = Join-Path $TestDrive 'formula-result.json'
    $csvPath = Join-Path $TestDrive 'formula-result.csv'
    Mock Write-Status {}
