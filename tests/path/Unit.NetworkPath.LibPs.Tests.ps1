...
BeforeAll {
  ...
  . "$PSScriptRoot/../../src/powershell/path/lib-ps/Test-TcpPort.ps1"
  ...
  function Test-NetConnection {
    ...
    $null = $ComputerName, $Port, $TraceRoute, $InformationLevel, $Hops
  }
}
...
Describe 'Test-TcpPort' {
  It 'uses the TCP parameter set without traceroute-only arguments' {
    Mock Get-HostAddressesWithTimeout { [System.Net.IPAddress]::Parse('192.0.2.10') }
    Mock Test-NetConnection {
      [pscustomobject]@{ TcpTestSucceeded = $true }
    }

    $result = Test-TcpPort -HostName 'example.test' -Port 443 -Protocol IPv4

    $result.TcpTestSucceeded | Should -BeTrue
    Should -Invoke Test-NetConnection -Times 1 -Exactly -ParameterFilter {
      $ComputerName -eq '192.0.2.10' -and
      $Port -eq 443 -and
      $InformationLevel -eq 'Detailed' -and
      -not $TraceRoute -and
      $null -eq $Hops
    }
  }
