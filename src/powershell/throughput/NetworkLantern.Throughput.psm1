...
# Default parameter set for Measure-NetworkThroughput (single source for CLI merge)
...
$privateScripts = @(
  ...
  'Profiles.ps1'
  'Reporting.ps1'
  'Iperf3Invocation.ps1'
  'Iperf3TestExecution.ps1'
  'Orchestration.ps1'
)
$missing = @($privateScripts | Where-Object { -not (Test-Path -LiteralPath (Join-Path $privateDir $_)) })
if ($missing.Count -gt 0) {
  throw "NetworkLantern.Throughput: Missing private script(s): $($missing -join ', '). Path: $privateDir"
}
...
function Measure-NetworkThroughput {
  ...
  .EXAMPLE
  Measure-NetworkThroughput -Target 'iperf3.example.com' -Port 5201
  .EXAMPLE
  Measure-NetworkThroughput -Target '10.0.0.1' -SingleTest -PassThru
  .EXAMPLE
  $summary = Measure-NetworkThroughput -Target '10.0.0.1' -Protocol TCP -DscpClasses @('CS0','EF') -PassThru
  #>
  ...
  # Use terminating errors during the run and restore the caller's preference afterward.
...
function Compare-Iperf3Runs {
  ...
  .DESCRIPTION
  Reads two summary JSON files (produced by Measure-NetworkThroughput) and computes
...
Export-ModuleMember -Function Measure-NetworkThroughput, Get-NetworkThroughputDefaultParameterSet, Get-Iperf3ProfileNames, Get-Iperf3ProfileParameters, Save-Iperf3Profile, Remove-Iperf3Profile, Compare-Iperf3Runs
