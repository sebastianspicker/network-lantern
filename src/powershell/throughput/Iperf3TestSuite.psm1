...
function Compare-Iperf3Runs {
  ...
  differences in test counts, status, and aggregate throughput. Useful for CI
  regression detection: compare a baseline run against the current run.
  .PARAMETER BaselinePath
  Path to the baseline (previous) summary JSON file.
  .PARAMETER CurrentPath
  Path to the current summary JSON file.
  .OUTPUTS
  [pscustomobject] Delta object with baseline/current status, counts, and changes.
  .EXAMPLE
