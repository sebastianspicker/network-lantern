function Reset-UjBaseline {
  ...
  if (-not $DryRun -and $PSCmdlet.ShouldProcess($mmKey, 'Remove registry tweaks')) {
    ...
    if (Test-Path -Path $audioKey) {
      foreach ($valueName in $script:UjOwnedMmcssAudioValueNames) {
        Remove-ItemProperty -Path $audioKey -Name $valueName -ErrorAction SilentlyContinue
      }
    }
  } elseif ($DryRun) {
    Write-UjInformation -Message '[DryRun] Remove registry tweaks owned by this tool (Throttling/Responsiveness/AFD/MMCSS Audio values)'
  }
